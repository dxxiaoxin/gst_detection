...
namespace RectTools
{
	...
	template <typename t>
	inline cv::Point2d getCenter(const cv::Rect_<t > &rect)
	{
		cv::Point2d res;
		res.x = rect.x + cvRound(rect.width / 2.0);
		res.y = rect.y + cvRound(rect.height / 2.0);
		return res;
	}
	...
	inline cv::Mat getSobleImage(cv::Mat img)
	{
		cv::cvtColor(img, img, CV_BGR2GRAY);
		//img.convertTo(img, CV_32F, 1 / 255.f);
		...
		float kernelx[9] = {-1.f, -2.f, -1.f, 0.f, 0.f, 0.f, 1.f, 2.f, 1.f};
		float kernely[9] = {-1.f, 0.f, 1.f, -2.f, 0.f, 2.f, -1.f, 0.f, 1.f};
		CvMat kernel_dx = cvMat(3, 3, CV_32F, kernelx);
		CvMat kernel_dy = cvMat(3, 3, CV_32F, kernely);
		...
		dx = cvCreateImage(cvSize(width, height), IPL_DEPTH_32F, numChannels);
		dy = cvCreateImage(cvSize(width, height), IPL_DEPTH_32F, numChannels);

		cvFilter2D(&image, dx, &kernel_dx, cvPoint(-1, -1));
		cvFilter2D(&image, dy, &kernel_dy, cvPoint(-1, -1));
		cv::Mat ddx = cv::cvarrToMat(dx);
		cv::Mat ddy = cv::cvarrToMat(dy);

		cv::Mat sobelimg;
		sobelimg.push_back(ddx);
		sobelimg.push_back(ddy);
		cvReleaseImage(&dx);
		cvReleaseImage(&dy);
		//sobelimg = sobelimg.reshape(1,1);
		//sobelimg.t();
		double max,min;
		cv::minMaxIdx(sobelimg, &min, &max);
		float ff = std::max(abs(min),abs(max));
		//sobelimg.convertTo(sobelimg, CV_32F, 1 / ff);
		sobelimg *= 1/ff;
	//	cv::imshow("sobel", sobelimg);
		return sobelimg.reshape(1,2);
	}
